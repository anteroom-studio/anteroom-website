...
function closeInspection() {
  ...
  slabLayer
    .querySelectorAll('.artifact-slab')
...
function renderSlabs(show) {
  ...
  slabLayer.innerHTML = archiveSlabs
    .map((s, i) => {
      return (
        '<button class="artifact-slab slab-' +
        i +
        '" data-i="' +
        i +
        '">' +
...
window.addEventListener('keydown', e => {
  if (e.key === 'Escape' && inspecting) closeInspection();
});
...
function updateUI(r) {
  ...
  ui.innerHTML =
    ...
    r.number +
    '</span>' +
    '<div class="bar"><i style="--progress:' +
    ((current + 1) / rooms.length) * 100 +
    '%"></i></div>' +
    '<span>' +
    rooms.length +
    '</span>' +
    ...
    '<div class="room-note">' +
    '<b>Studio record</b>' +
    '<span>The interface reveals only what the room is ready to show.</span>' +
    '</div>';
